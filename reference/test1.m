...
%% 理论解
U1 = @(x, y)exp(1j*k*(z + (x.^2 + y.^2) / (2 * z))) / (1j * lambda * z) * a * b .* sinc(a*x/(lambda * z)) .* sinc(b*y/(lambda * z));
U1 = U1(X, Y);
work(X, Y, U1, "理论解");

%% 数值解
[~, U2] = fraunhofer_fft(@(x, y)ones(size(x)), xmin, xmax, ymin, ymax, nn, nn, lambda, z, Xmin, Xmax, Ymin, Ymax, nn, nn);
work(X, Y, U2, "数值解");
